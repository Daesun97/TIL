...
# 활용하시오.(답안 예시) 1
# =============================================================================
#DF.apply(함수명또는 lambda사용해서 함수를 제작가능함)
q1=data11.groupby('Country').apply(len)

sel_list=q1[q1== 3].index

(q1 < 3).sum()
#답:20


q1_2=data11[data11.Country.isin(sel_list)]
...
# =============================================================================
tab= data11.pivot_table(values='Happiness_Score',index='Country', columns='year' )

tab.isna().any(axis=1).sum()

q2= tab.dropna()



q2.loc[:,'ratio']=(q2[2017]-q2[2015]) / 2
...
from statsmodels.formula.api import ols
from statsmodels.stats.anova import anova_lm # 집단간의 평균 차이유무
...
q3= data11[data11.Country.isin(sel_list)]

ols_out=ols('Happiness_Score~C(year)',q3).fit()

anova_lm(ols_out)['F'][0]

#답 : 0.0042

compare_out=pairwise_tukeyhsd(q3['Happiness_Score'], q3['year'])
...
# 제품의 판매 개수는? (답안 예시) 1
# =============================================================================
search_date=pos1['Date'].value_counts().idxmax()
# '2015-01-21'
q1=pos1[pos1.Date==search_date]['itemDescription'].value_counts().nlargest(1)
...
pos1['Date'].str.split('-')
pos2=pos1.copy()
pos2['month']=pd.to_datetime(pos1['Date']).dt.month # year, day 다됨

pos2['day_name']=pd.to_datetime(pos1['Date']).dt.day_name(locale='ko_kr') # 요일 호출 
...
merge2.columns
q2=pd.pivot_table(merge2, index='Date', columns='week', values='alcohol',aggfunc='sum')


from scipy.stats import ttest_ind

q2_out=ttest_ind(q2[0].dropna(),q2[1].dropna(), equal_var=False)
...
q3_2=q3.groupby(['Date','day_name']).apply(len).reset_index()
q3_2.columns=['Date', 'day_name','freq'] #0때문에 안되서 바꿔야함
q3_3=pd.pivot_table(q3, index=['Date','day_name'],
...
anova_out=ols('freq~day_name', q3_3).fit()


round(anova_lm(anova_out)['PR(>F)'][0],2)
