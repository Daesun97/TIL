...
pvalue_out=ols1.pvalues.drop('Intercept')

q3_out1=pvalue_out[pvalue_out <0.05]

len(q3_out1)
# 답 : 13

(ols1.params[q3_out1.index] < 0).sum() 
# 답 : 2
...
q1_corr = data7[var_list].corr().drop('Chance_of_Admit')['Chance_of_Admit'].abs()
...
ttest_out=ttest_ind(q2[q2.GRE_gr ==1]['CGPA'],
          q2[q2.GRE_gr ==0]['CGPA'],
          equal_var=True)
...
var_list=q3.columns.drop(['Serial_No','Chance_of_Admit','group'])

from sklearn.linear_model import LogisticRegression

logit= LogisticRegression(random_state=123, solver= 'liblinear')
logit.fit(q3[var_list], q3['group'])
q3_out=pd.DataFrame(logit.coef_.T)
q3_out.index=var_list

q3_out[0].abs().nlargest(1)
...
q3_out['state'][q3_out['mape'].argmin()]

round(q3_out['mape'].min(),2)
...
data9.columns
#['satisfaction', 'Gender', 'Age', 'Customer_Type', 'Class',
...
for i in var_list:
    ...
    q2_out.append([i, chi2_out[1],chi2_out[0]])
    
q2_out=pd.DataFrame(q2_out, columns=['var','pvalue','chi2'])

int(q2_out[q2_out.pvalue <0.05]['chi2'])
...
train, test= train_test_split(data9.dropna(), test_size=0.3, random_state=123)
...
round(f1_score(test['satisfaction'],pred, pos_label='dissatisfied'),4)
...
data10=data10.dropna(how='all',axis=1) #빈칸만 다날림
#%%
...
sel_model_list=[tab['km_per_day'].idxmax(),tab['km_per_day'].idxmin()]

max_df=q2[q2.model == sel_model_list[0]]['km_per_day']
min_df=q2[q2.model == sel_model_list[1]]['km_per_day']

from scipy.stats import ttest_ind

ttest_out=ttest_ind(max_df, min_df, equal_var=True)
