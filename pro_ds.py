...
q2.argmax()# 최대값의 위치번호 리턴
q2.nlargest(1)#최대값과 인댁스명
q2.quantile(80)# 상위 20%를 찾는것
 # nlargest.index에 붙여서 인덱스값 들고옴
...
from sklearn.linear_model import LinearRegression  #array
from statsmodels.formula.api import ols # 자동적으로 범주형범수를 더미로 전환시켜줌, 분산분석 가능
...
#-1(상수항제외) 
#C()범주형데이터
#ols('Sales~TV+Radio+Social_Media', data1)fit() 이렇게 되야함
...
#np.round(q3_out.sort_values(ascending=False).values, 3)
#np.ceil(q3_out.sort_values(ascending=False).values * 1000)/1000
#np.floor(q3_out.sort_values(ascending=False).values * 1000)/1000
q3_out.sort_values(ascending=False).values
#값 =[3.562,  0.004, -0.003] 
(q3.outlier_test() < 0.05)
#data1[q3.outlier_test()[bonf(p)] < 0.05]#이상치 있음
#data1[q3.outlier_test()[bonf(p)] >= 0.05]#이상치 없음
...
data2.columns
#['Age', 'Sex', 'BP', 'Cholesterol', 'Na_to_K', 'Drug'], dtype='object'
#%%
...
#2. 카이스퀘어 검정
 # (1) 빈도구성
tab=pd.crosstab(index=q2['Sex'], columns=['Drug'])
...
#변수생성 : Sex_cd, BP_cd, Ch_cd
q3=data2.copy()

q3['Sex_cd']=np.where(q3.Sex == 'M',0,1)
q3['BP_cd']=np.where(q3.BP== 'LOW',0,np.where(q3.BP=='NORMLA',1,2))
...
#Root Node 에서 선택된 변수 (split feature)와 분리기준
plot_tree(dt,feature_names=var_list, max_depth=2, class_names=list(q3['Drug'].unique()),
...
from scipy.stats import bartlett, levene #등분산여부 검정 여기서는 필요없음

#ttest_1samp: 1표본 t검정
#ttest_ind: 독립인 2표본 t검정
#ttest_rel: 대응인 2표번 t검정
...
round(abs(q2_out.statistic),3) # 소수점 3째자리까지 반올림

#신뢰수준 99%에서 양측 검정을 수행, 결과는 귀무가설 기각의 경우 Y, 그렇지 않으면 N
